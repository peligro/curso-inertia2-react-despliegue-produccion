...
const Edit = () => {
     const { perfiles, errors, datos, flash } = usePage<UsuariosFormularioProps>().props
    const { data, setData, post, processing } = useForm({
        perfil_id:`${datos?.perfiles_id}`,
        nombre: datos?.users?.name,
        correo: datos?.users?.email,
        password: '',
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        post(route('usuarios_edit_post', {id: datos?.id}))
    }
            ...
            <div className="row">
                <div className="col-12">
                    <Breadcrumb>
                        ...
                        <Breadcrumb.Item active>Editar: <strong>{datos?.users?.name}</strong></Breadcrumb.Item>
                    </Breadcrumb>

                    <MensajesFlash flash={flash} />
                    <h1>Editar: <strong>{datos?.users?.name}</strong></h1>
